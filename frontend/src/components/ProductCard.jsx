...
export default function ProductCard({ product }) {
  ...
  const p = product || {};
  const hasVariants = Array.isArray(p.variants?.storage) && p.variants.storage.length > 0;
  ...
  function handleWhatsApp(e) {
    e.stopPropagation();
    window.open(whatsAppProductLink(p, {}), "_blank");
  }
          ...
          <div className="relative aspect-square overflow-hidden bg-gradient-to-br from-ink/5 to-ink/10">
              {imgError ? (
                <div className="flex h-full w-full items-center justify-center p-4 text-center">
                  <span className="font-display text-sm font-700 text-ink/70">{p.name}</span>
                </div>
              ) : (
                <img
              ...
              <div className="absolute left-3 top-3 flex flex-col gap-1.5">
                ...
                {p.oldPrice && (
                  <span className="w-fit rounded-full bg-gold px-3 py-1 text-[11px] font-700 uppercase tracking-wide text-ink shadow-sm">
                    Save {formatGHS(p.oldPrice - p.price)}
                  </span>
                )}
              </div>
            ...
            <div className="flex flex-1 flex-col gap-1 p-4">
              ...
              <h3 className="font-display text-base font-700 text-ink leading-snug">{p.name}</h3>
              <p className="line-clamp-1 text-xs text-ink/50">{p.spec}</p>

              {lowStock && !outOfStock && (
                <p className="text-[11px] font-600 text-signal">Only {p.stock} left in stock</p>
              )}
              {outOfStock && <p className="text-[11px] font-600 text-ink/40">Out of stock</p>}

              <div className="mt-3 flex flex-col gap-1.5">
                <div>
                  <p className="font-display text-base font-700 text-ink sm:text-lg">{formatGHS(p.price)}</p>
                  {p.oldPrice && (
                    <p className="text-xs text-ink/40 line-through">{formatGHS(p.oldPrice)}</p>
                  )}
                </div>
