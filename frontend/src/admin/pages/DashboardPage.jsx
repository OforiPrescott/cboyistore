...
export default function DashboardPage() {
  ...
  const revenue = useMemo(() => paidOrders.reduce((s, o) => s + o.total, 0), [paidOrders]);
  const linkedOrders = useMemo(() => orders.filter((o) => Boolean(o.userId)), [orders]);
  ...
  const orderRate = useMemo(
    () => (orders.length ? Math.round((linkedOrders.length / orders.length) * 100) : 0),
    [orders, linkedOrders]
  );
            ...
            <Card>
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                ...
                <div className="rounded-3xl border border-ink/10 bg-cream p-5">
                  ...
                  <p className="mt-2 text-2xl font-700 text-ink">{linkedAccounts}</p>
